...
from game.__settings__ import FRUIT_DICT, ICECUBE_IMAGE, BOMB_IMAGE, ICECUBE_COLOR, BOMB_COLOR
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN and not self.current_player.alive():
            ...
            if self.frame % self.spawn_delay == 0:
                if not self.current_player.frozen() and self.current_player.alive():
                    if secrets.randbelow(100) > self.spawn_delay:
                        fruit = self.create_fruits('fruit')
                        self.fruits.append(fruit)
                    if secrets.randbelow(100) > self.spawn_delay + 50:
                        fruit = self.create_fruits('icecube')
                        self.fruits.append(fruit)
                    if secrets.randbelow(100) > self.spawn_delay + 60:
                        fruit = self.create_fruits('bomb')
                        self.fruits.append(fruit)
                    
            self.frame = self.clock_tick(self.frame)

    def create_fruits(self, type):
        ...
        fruit = Fruits(random_x_position, (self.screen.height+1), random_size, image, random_rotation, random_letter, color, self.screen, type)
        return fruit